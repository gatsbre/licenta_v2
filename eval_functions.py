def precision_score(test_set, predictions, k=10):
    ...
    for user in set_of_users:
        list_of_predictions = [
            prediction for prediction in predictions if prediction.uid == user
        ]
        sorted_predictions = sorted(
            list_of_predictions, key=lambda x: x.est, reverse=True
        )
        ...
        relevant_items_count = sum(
            ...
            if abs(prediction.r_ui - prediction.est) >= 0.5
        )
